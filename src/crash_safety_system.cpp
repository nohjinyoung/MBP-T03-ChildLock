...
public:
    void processSafetyProtocol(const VehicleContext& ctx) {
        ...
        // --- 상황 C: 전원 상태 수치 검증 ---
        std::cout << "  [DATA] 전원 모니터링: Main=" << ctx.main_vcc << "V, Backup=" << ctx.cap_voltage << "V" << std::endl;
        if (ctx.main_vcc < VOLTAGE_CRITICAL) {
            std::cout << "  [CRITICAL] 메인 전원 상실 감지 (V < " << VOLTAGE_CRITICAL << "V)" << std::endl;
        ...
        // --- 상황 A: 충돌 센서 수치 검증 ---
        std::cout << "  [DATA] 충돌 감지: Airbag=" << (ctx.airbag_signal ? "ON" : "OFF") 
                  << ", Accel_L=" << ctx.accel_l << "G, Accel_R=" << ctx.accel_r << "G" << std::endl;

        bool is_physics_valid = (ctx.accel_l > G_THRESHOLD) && (ctx.accel_r > G_THRESHOLD);
        ...
        if (ctx.airbag_signal && is_physics_valid) {
            ...
            // --- 상황 B: 후방 위험 수치 검증 ---
            if (ctx.rear_hazard) {
                std::cout << "  [DATA] 후방 장애물 접근 감지: 거리 " << ctx.hazard_dist << "m" << std::endl;
                logAction("외부 경고 활성화: 비상등/실내등 Max-Intensity 점멸 (상황 B 대응)");
            }
        } else {
            // 노이즈 상황 분석 로그
            if (!is_physics_valid && (ctx.accel_l > G_THRESHOLD || ctx.accel_r > G_THRESHOLD)) {
                std::cout << "  [ANALYSIS] 편차 발생: 단일 센서 임계값 초과(노이즈 가능성 99%)." << std::endl;
            }
