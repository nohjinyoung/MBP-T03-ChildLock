...
private:
    ...
    static void logPowerData(const VehicleContext& ctx)
    {
        std::cout << "  [DATA] 전원 모니터링: Main=" << ctx.main_vcc
                  << "V, Backup=" << ctx.cap_voltage << "V" << std::endl;
    }

    bool handlePowerLoss(const VehicleContext& ctx) const
    {
        logPowerData(ctx);
    ...
    static void logCrashData(const VehicleContext& ctx)
    {
    ...
    bool isPhysicsValid(const VehicleContext& ctx) const
    {
    ...
    void handleNoiseCase(const VehicleContext& ctx, bool is_physics_valid) const
    {
        if (!is_physics_valid &&
...
public:
    void processSafetyProtocol(const VehicleContext& ctx) const
    {
        ...
        if (handlePowerLoss(ctx)) {
            return;
        }

        logCrashData(ctx);
        const bool is_physics_valid = isPhysicsValid(ctx);

        if (ctx.airbag_signal && is_physics_valid) {
            logAction("Crash Unlock 조건 충족: 도어 잠금 해제 실행");
            handleRearHazard(ctx);
            return;
        }

        handleNoiseCase(ctx, is_physics_valid);
    }
};
